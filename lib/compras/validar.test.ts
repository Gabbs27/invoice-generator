...
const validar = (compra: Compra) => validarCompra(compra, NEGOCIO);

describe('validar una compra', () => {
  it('acepta una compra completa', () => {
    expect(validar(compraDePrueba())).toEqual([]);
  });

  it('acepta una cédula de proveedor', () => {
    expect(validar(compraDePrueba({ RNCCedula: '00100000001' }))).toEqual([]);
  });

  it('rechaza un RNC que no tiene 9 u 11 dígitos', () => {
    expect(validar(compraDePrueba({ RNCCedula: '12345678' }))).toEqual([
  ...
  it('acepta una retención de ISR completa', () => {
    expect(
      validar(compraDePrueba({ MontoRetencionRenta: '100.00', TipoRetencionISR: '2', FechaPago: '20260920' }))
    ).toEqual([]);
  });

  it('no acepta un tipo de retención sin monto retenido', () => {
    expect(validar(compraDePrueba({ TipoRetencionISR: '1' }))).toEqual([
      expect.stringMatching(/va con un monto retenido/),
    ]);
  });

  it('rechaza una forma de pago fuera del 1 al 7', () => {
    expect(validar(compraDePrueba({ FormaPago: '8' as FormaPago }))).toEqual([
  ...
  it('pide el RNC del negocio en un comprobante de gastos menores', () => {
    expect(validar(compraDePrueba({ NCF: 'B1300000001' }))).toEqual([
      expect.stringMatching(/gastos menores.*101010101/),
    ]);
    expect(validar(compraDePrueba({ NCF: 'E430000000001' }))).toEqual([
